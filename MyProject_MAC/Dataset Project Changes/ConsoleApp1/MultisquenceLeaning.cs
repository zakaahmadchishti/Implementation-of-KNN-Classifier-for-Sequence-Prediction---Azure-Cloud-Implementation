...
//As the speed remain the same so, we have to follow the previous data
...
namespace NeoCortexApiSample
{
    ...
    public class MultiSequenceLearning
    {
        ...
        {
            string filePath = "active_cells.json";
            ...
            // Read the existing sequences from the file, if it exists
            ...
            // Find the last occurrence of the sequence name and insert the new sequence right after it
            int insertIndex = lines.FindLastIndex(line => line.Contains($"\"SequenceName\":\"{sequenceName}\""));
            if (insertIndex == -1)
            {
                // If the sequence name is not found, append the new sequence at the end
                lines.Add(jsonString);
            }
            else
            {
                // If found, insert the new sequence after the last occurrence
                lines.Insert(insertIndex + 1, jsonString);
            }
